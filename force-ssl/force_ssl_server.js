...
httpServer.addListener('request', function (req, res) {
  ...
  // Determine if the connectio was over SSL at any point. Either we
  ...
  var isSsl = req.connection.pair ||
      (req.headers['x-forwarded-proto'] &&
       ...
       // dont' apply SSL orders endpoing
       var isExclude = req.url.includes('/email-orders') || req.url.includes('/orders') || req.url.includes('/handler');

       if (!isLocal && !isSsl && !isExclude) {
